...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['action'] === 'add') {
        ...
        $id = $_POST['id'];
        $old = $pdo->prepare("SELECT status, notifications FROM todos WHERE id = ? AND user_id = ?");
        ...
        if ($status === 'completed' && $old['status'] !== 'completed' && $notifications) {
            ...
            if ($email && function_exists('simulate_email')) {
                simulate_email(
                    to: $email,
                    subject: "Task Done!",
                    message: "Task '" . htmlspecialchars($title, ENT_QUOTES, 'UTF-8') . "' completed!"
                );
            }
        }
...
<body>
    ...
    <div class="container mt-5">
        ...
        <form method="GET" class="row g-2 mb-3">
            ...
            <div class="col-md-3"><select name="category" class="form-select">
                    <option value="">All
                        Categories</option>
            ...
            <ul class="pagination">
                <?php for ($i = 1; $i <= $pages; $i++): ?>
                    <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>"><a class="page-link"
                            href="?page=<?php echo $i; ?>&<?php echo http_build_query([
                                                                'search' => $_GET['search'] ?? '',
                                                                'status' => $_GET['status'] ?? '',
